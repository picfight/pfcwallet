...
func (b *rpcBackend) GetHeaders(ctx context.Context, blockLocators []*chainhash.Hash, hashStop *chainhash.Hash) ([][]byte, error) {
	const op errors.Op = "pfcd.jsonrpc.getheaders"
...
func (b *rpcBackend) LoadTxFilter(ctx context.Context, reload bool, addrs []pfcutil.Address, outpoints []wire.OutPoint) error {
	const op errors.Op = "pfcd.jsonrpc.loadtxfilter"
...
func (b *rpcBackend) PublishTransaction(ctx context.Context, tx *wire.MsgTx) error {
	const op errors.Op = "pfcd.jsonrpc.sendrawtransaction"
...
func (b *rpcBackend) AddressesUsed(ctx context.Context, addrs []pfcutil.Address) (bitset.Bytes, error) {
...
func (b *rpcBackend) Rescan(ctx context.Context, blocks []chainhash.Hash) ([]*wallet.RescannedBlock, error) {
	const op errors.Op = "pfcd.jsonrpc.rescan"
...
func (b *rpcBackend) StakeDifficulty(ctx context.Context) (pfcutil.Amount, error) {
	const op errors.Op = "pfcd.jsonrpc.getstakedifficulty"

	r, err := b.rpcClient.GetStakeDifficulty()
	if err != nil {
		return 0, errors.E(op, err)
	}
	amount, err := pfcutil.NewAmount(r.NextStakeDifficulty)
	if err != nil {
		return 0, errors.E(op, err)
	}
	return amount, nil
...
func (b *rpcBackend) GetBlockHash(ctx context.Context, height int32) (*chainhash.Hash, error) {
	const op errors.Op = "pfcd.jsonrpc.getblockhash"
