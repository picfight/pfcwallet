...
func (s *NeutrinoClient) GetBlock(hash *chainhash.Hash) (*wire.MsgBlock, error) {
	...
	//  Should the block cache be INSIDE neutrino instead of in pfcwallet?
...
func (s *NeutrinoClient) Rescan(startHash *chainhash.Hash, addrs []pfcutil.Address,
...
func (s *NeutrinoClient) NotifyBlocks() error {
	...
	if !s.scanning {
		s.clientMtx.Unlock()
		return s.NotifyReceived([]pfcutil.Address{})
	}
	s.clientMtx.Unlock()
	return nil
}
...
func (s *NeutrinoClient) NotifyReceived(addrs []pfcutil.Address) error {
	s.clientMtx.Lock()
...
func (s *NeutrinoClient) onFilteredBlockConnected(height int32,
	header *wire.BlockHeader, relevantTxs []*pfcutil.Tx) {
