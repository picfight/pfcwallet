...
// pfcd RPC chain server, or an SPV library, as long as we write a driver for
// it.
type Interface interface {
	...
	Rescan(*chainhash.Hash, []pfcutil.Address, map[wire.OutPoint]pfcutil.Address) error
	NotifyReceived([]pfcutil.Address) error
	NotifyBlocks() error
...
type (
	...
	FilterBlocksRequest struct {
		...
		ExternalAddrs    map[waddrmgr.ScopedIndex]pfcutil.Address
		InternalAddrs    map[waddrmgr.ScopedIndex]pfcutil.Address
	...
	FilterBlocksResponse struct {
		...
		FoundOutPoints     map[wire.OutPoint]pfcutil.Address
