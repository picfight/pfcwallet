...
// PicfightcoindConn represents a persistent client connection to a picfightcoind node
// that listens for events read from a ZMQ connection.
type PicfightcoindConn struct {
	started int32 // To be used atomically.
	stopped int32 // To be used atomically.

	// rescanClientCounter is an atomic counter that assigns a unique ID to
	// each new picfightcoind rescan client using the current picfightcoind
	// connection.
	rescanClientCounter uint64

	// chainParams identifies the current network the picfightcoind node is
	// running on.
	chainParams *chaincfg.Params

	// client is the RPC client to the picfightcoind node.
	...
	// rescanClients is the set of active picfightcoind rescan clients to which
	// ZMQ event notfications will be sent to.
	rescanClientsMtx sync.Mutex
	rescanClients    map[uint64]*PicfightcoindClient

	quit chan struct{}
	wg   sync.WaitGroup
}

// NewPicfightcoindConn creates a client connection to the node described by the host
// string. The ZMQ connections are established immediately to ensure liveness.
// If the remote node does not operate on the same picfightcoin network as described
// by the passed chain parameters, the connection will be disconnected.
func NewPicfightcoindConn(chainParams *chaincfg.Params,
	host, user, pass, zmqBlockHost, zmqTxHost string,
	zmqPollInterval time.Duration) (*PicfightcoindConn, error) {
	...
	// Establish two different ZMQ connections to picfightcoind to retrieve block
	...
	conn := &PicfightcoindConn{
		chainParams:   chainParams,
		client:        client,
		zmqBlockConn:  zmqBlockConn,
		zmqTxConn:     zmqTxConn,
		rescanClients: make(map[uint64]*PicfightcoindClient),
...
// Start attempts to establish a RPC and ZMQ connection to a picfightcoind node. If
...
func (c *PicfightcoindConn) Start() error {
...
// Stop terminates the RPC and ZMQ connection to a picfightcoind node and removes any
// active rescan clients.
func (c *PicfightcoindConn) Stop() {
...
func (c *PicfightcoindConn) blockEventHandler() {
	defer c.wg.Done()

	log.Info("Started listening for picfightcoind block notifications via ZMQ "+
	...
	for {
		...
		default:
			...
			// picfightcoind shuts down, which will produce an unreadable
...
//
// NOTE: This must be run as a goroutine.
func (c *PicfightcoindConn) txEventHandler() {
	defer c.wg.Done()

	log.Info("Started listening for picfightcoind transaction notifications "+
...
// getCurrentNet returns the network on which the picfightcoind node is running.
func (c *PicfightcoindConn) getCurrentNet() (wire.PicfightcoinNet, error) {
...
// NewPicfightcoindClient returns a picfightcoind client using the current picfightcoind
// connection. This allows us to share the same connection using multiple
// clients.
func (c *PicfightcoindConn) NewPicfightcoindClient() *PicfightcoindClient {
	return &PicfightcoindClient{
		quit: make(chan struct{}),
...
func (c *PicfightcoindConn) AddClient(client *PicfightcoindClient) {
	c.rescanClientsMtx.Lock()
...
//
// NOTE: This function is safe for concurrent access.
func (c *PicfightcoindConn) RemoveClient(id uint64) {
	c.rescanClientsMtx.Lock()
	defer c.rescanClientsMtx.Unlock()
