...
var (
	// ErrPicfightcoindClientShuttingDown is an error returned when we attempt
	// to receive a notification for a specific item and the picfightcoind client
	// is in the middle of shutting down.
	ErrPicfightcoindClientShuttingDown = errors.New("client is shutting down")
)

// PicfightcoindClient represents a persistent client connection to a picfightcoind server
// for information regarding the current best block chain.
type PicfightcoindClient struct {
	...
	// id is the unique ID of this client assigned by the backing picfightcoind
	// connection.
	id uint64

	// chainConn is the backing client to our rescan client that contains
	// the RPC and ZMQ connections to a picfightcoind node.
	chainConn *PicfightcoindConn
	...
	// retrieved from the backing picfightcoind connection.
...
// A compile-time check to ensure that PicfightcoindClient satisfies the
// chain.Interface interface.
var _ Interface = (*PicfightcoindClient)(nil)

// BackEnd returns the name of the driver.
func (c *PicfightcoindClient) BackEnd() string {
	return "bitcoind"
}

// GetBestBlock returns the highest block known to picfightcoind.
func (c *PicfightcoindClient) GetBestBlock() (*chainhash.Hash, int32, error) {
...
// error.
func (c *PicfightcoindClient) GetBlockHeight(hash *chainhash.Hash) (int32, error) {
...
func (c *PicfightcoindClient) GetBlock(hash *chainhash.Hash) (*wire.MsgBlock, error) {
	return c.chainConn.client.GetBlock(hash)
}

// GetBlockVerbose returns a verbose block from the hash.
func (c *PicfightcoindClient) GetBlockVerbose(
	hash *chainhash.Hash) (*pfcjson.GetBlockVerboseResult, error) {

	return c.chainConn.client.GetBlockVerbose(hash)
}

// GetBlockHash returns a block hash from the height.
func (c *PicfightcoindClient) GetBlockHash(height int64) (*chainhash.Hash, error) {
	return c.chainConn.client.GetBlockHash(height)
}

// GetBlockHeader returns a block header from the hash.
func (c *PicfightcoindClient) GetBlockHeader(
...
func (c *PicfightcoindClient) GetBlockHeaderVerbose(
	hash *chainhash.Hash) (*pfcjson.GetBlockHeaderVerboseResult, error) {
...
// as "current".
func (c *PicfightcoindClient) IsCurrent() bool {
...
func (c *PicfightcoindClient) GetRawTransactionVerbose(
	hash *chainhash.Hash) (*pfcjson.TxRawResult, error) {

	return c.chainConn.client.GetRawTransactionVerbose(hash)
}

// GetTxOut returns a txout from the outpoint info provided.
func (c *PicfightcoindClient) GetTxOut(txHash *chainhash.Hash, index uint32,
	mempool bool) (*pfcjson.GetTxOutResult, error) {

	return c.chainConn.client.GetTxOut(txHash, index, mempool)
}

// SendRawTransaction sends a raw transaction via picfightcoind.
func (c *PicfightcoindClient) SendRawTransaction(tx *wire.MsgTx,
...
func (c *PicfightcoindClient) Notifications() <-chan interface{} {
...
//
// NOTE: This is part of the chain.Interface interface.
func (c *PicfightcoindClient) NotifyReceived(addrs []pfcutil.Address) error {
	c.NotifyBlocks()

	select {
	case c.rescanUpdate <- addrs:
	case <-c.quit:
		return ErrPicfightcoindClientShuttingDown
	}
...
func (c *PicfightcoindClient) NotifySpent(outPoints []*wire.OutPoint) error {
	c.NotifyBlocks()
...
func (c *PicfightcoindClient) NotifyTx(txids []chainhash.Hash) error {
	c.NotifyBlocks()
...
//
// NOTE: This is part of the chain.Interface interface.
func (c *PicfightcoindClient) NotifyBlocks() error {
	...
	// picfightcoind connection in order to receive ZMQ event notifications for
...
func (c *PicfightcoindClient) shouldNotifyBlocks() bool {
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]pfcutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *PicfightcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	if reset {
		select {
	...
	for _, filter := range filters {
		...
		case []pfcutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]pfcutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
// matched as []pfcjson.BlockDetails.
func (c *PicfightcoindClient) RescanBlocks(
	blockHashes []chainhash.Hash) ([]pfcjson.RescannedBlock, error) {

	rescannedBlocks := make([]pfcjson.RescannedBlock, 0, len(blockHashes))
	for _, hash := range blockHashes {
		header, err := c.GetBlockHeaderVerbose(&hash)
		if err != nil {
			log.Warnf("Unable to get header %s from picfightcoind: %s",
				hash, err)
			continue
		}
		...
		if err != nil {
			log.Warnf("Unable to get block %s from picfightcoind: %s",
				hash, err)
			continue
		}
		...
		if len(relevantTxs) > 0 {
			rescannedBlock := pfcjson.RescannedBlock{
				Hash: hash.String(),
			}
...
func (c *PicfightcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []pfcutil.Address, outPoints map[wire.OutPoint]pfcutil.Address) error {
...
// Start initializes the picfightcoind rescan client using the backing picfightcoind
// connection and starts all goroutines necessary in order to process rescans
// and ZMQ notifications.
//
// NOTE: This is part of the chain.Interface interface.
func (c *PicfightcoindClient) Start() error {
...
// Stop stops the picfightcoind rescan client from processing rescans and ZMQ
// notifications.
//
// NOTE: This is part of the chain.Interface interface.
func (c *PicfightcoindClient) Stop() {
	...
	// Remove this client's reference from the picfightcoind connection to
...
//
// NOTE: This is part of the chain.Interface interface.
func (c *PicfightcoindClient) WaitForShutdown() {
	c.wg.Wait()
}
...
func (c *PicfightcoindClient) rescanHandler() {
	defer c.wg.Done()
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]pfcutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
...
// picfightcoind connection.
//
// NOTE: This must be called as a goroutine.
func (c *PicfightcoindClient) ntfnHandler() {
	defer c.wg.Done()
...
// SetBirthday sets the birthday of the picfightcoind rescan client.
//
// NOTE: This should be done before the client has been started in order for it
// to properly carry its duties.
func (c *PicfightcoindClient) SetBirthday(t time.Time) {
	c.birthday = t
}

// BlockStamp returns the latest block notified by the client, or an error
// if the client has been shut down.
func (c *PicfightcoindClient) BlockStamp() (*waddrmgr.BlockStamp, error) {
	c.bestBlockMtx.RLock()
	bestBlock := c.bestBlock
	c.bestBlockMtx.RUnlock()
...
func (c *PicfightcoindClient) onBlockConnected(hash *chainhash.Hash, height int32,
	timestamp time.Time) {
...
func (c *PicfightcoindClient) onFilteredBlockConnected(height int32,
...
func (c *PicfightcoindClient) onBlockDisconnected(hash *chainhash.Hash, height int32,
	timestamp time.Time) {
...
// caller.
func (c *PicfightcoindClient) onRelevantTx(tx *wtxmgr.TxRecord,
...
func (c *PicfightcoindClient) onRescanProgress(hash *chainhash.Hash, height int32,
	timestamp time.Time) {
...
func (c *PicfightcoindClient) onRescanFinished(hash *chainhash.Hash, height int32,
	timestamp time.Time) {
...
func (c *PicfightcoindClient) reorg(currentBlock waddrmgr.BlockStamp,
...
//
// NOTE: This is part of the chain.Interface interface.
func (c *PicfightcoindClient) FilterBlocks(
...
// rescan performs a rescan of the chain using a picfightcoind backend, from the
...
// loop.
func (c *PicfightcoindClient) rescan(start chainhash.Hash) error {
	...
	// Cycle through all of the blocks known to picfightcoind, being mindful of
	// reorgs.
	for i := previousHeader.Height + 1; i <= bestBlock.Height; i++ {
			...
			if headers.Back() != nil {
				...
				if headers.Back() != nil {
					previousHeader = headers.Back().
						Value.(*pfcjson.GetBlockHeaderVerboseResult)
				...
				// Otherwise, we get it from picfightcoind.
				previousHash, err = chainhash.NewHashFromStr(
		...
		previousHeader = &pfcjson.GetBlockHeaderVerboseResult{
...
func (c *PicfightcoindClient) filterBlock(block *wire.MsgBlock, height int32,
...
func (c *PicfightcoindClient) filterTx(tx *wire.MsgTx,
	blockDetails *pfcjson.BlockDetails,
	notify bool) (bool, *wtxmgr.TxRecord, error) {

	txDetails := pfcutil.NewTx(tx)
	if blockDetails != nil {
