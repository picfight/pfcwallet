...
type RPCClient struct {
	*pfcrpcclient.Client
	connConfig  *pfcrpcclient.ConnConfig // Work around unexported field
...
func NewRPCClient(chainParams *chaincfg.Params, connect, user, pass string, certs []byte,
	...
	ntfnCallbacks := &pfcrpcclient.NotificationHandlers{
	...
	rpcClient, err := pfcrpcclient.New(client.connConfig, ntfnCallbacks)
	if err != nil {
		return nil, err
	}
...
// pfcrpcclient callbacks, which isn't very Go-like and doesn't allow
...
// picfight RPC server.  This channel must be continually read or the process
...
// POSTClient creates the equivalent HTTP POST pfcrpcclient.Client.
func (c *RPCClient) POSTClient() (*pfcrpcclient.Client, error) {
	configCopy := *c.connConfig
	configCopy.HTTPPostMode = true
	client, err := pfcrpcclient.New(&configCopy, nil)
	if err != nil {
