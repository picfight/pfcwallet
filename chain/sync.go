...
// the pfcd server are performed.  Otherwise, it will listen for notifications
...
func (s *RPCSyncer) Run(ctx context.Context, startupSync bool) error {
	...
	g.Go(func() error {
		...
		if err != nil {
			const op errors.Op = "pfcd.jsonrpc.notifyspentandmissedtickets"
			return errors.E(op, err)
		}
		...
		if s.wallet.VotingEnabled() {
			...
			if err != nil {
				const op errors.Op = "pfcd.jsonrpc.notifywinningtickets"
				return errors.E(op, err)
			}
...
func (s *RPCSyncer) handleNotifications(ctx context.Context) error {
	...
	for {
		...
		case n, ok := <-c:
			...
			switch n := n.(type) {
			case blockConnected:
				op = "pfcd.jsonrpc.blockconnected"
			...
			case relevantTxAccepted:
				op = "pfcd.jsonrpc.relevanttxaccepted"
				nonFatal = true
				var rpt *chainhash.Hash
				rpt, err = s.wallet.RescanPoint()
				if err != nil || rpt != nil {
			...
			case missedTickets:
				op = "pfcd.jsonrpc.spentandmissedtickets"
...
func (s *RPCSyncer) handleVoteNotifications(ctx context.Context) error {
	...
	for {
		...
		case n, ok := <-c:
			...
			switch n := n.(type) {
			case winningTickets:
				op = "pfcd.jsonrpc.winningtickets"
...
func (s *RPCSyncer) startupSync(ctx context.Context) error {
	...
	if err != nil {
		const op errors.Op = "pfcd.jsonrpc.rebroadcastwinners"
		return errors.E(op, err)
	}
	_, err = s.rpcClient.RawRequest("rebroadcastmissed", nil)
	if err != nil {
		const op errors.Op = "pfcd.jsonrpc.rebroadcastmissed"
		return errors.E(op, err)
	}
