...
// RPCClient represents a persistent client connection to a picfightcoin RPC server
...
// operate on the same picfightcoin network as described by the passed chain
...
func (c *RPCClient) Rescan(startHash *chainhash.Hash, addrs []pfcutil.Address,
	outPoints map[wire.OutPoint]pfcutil.Address) error {
...
// picfightcoin RPC server.  This channel must be continually read or the process
...
func parseBlock(block *pfcjson.BlockDetails) (*wtxmgr.BlockMeta, error) {
	if block == nil {
		return nil, nil
	}
...
func (c *RPCClient) onRecvTx(tx *pfcutil.Tx, block *pfcjson.BlockDetails) {
...
func (c *RPCClient) onRedeemingTx(tx *pfcutil.Tx, block *pfcjson.BlockDetails) {
