...
type TicketPurchaser struct {
	...
	pfcdChainSvr     *pfcrpcclient.Client
...
func (t *TicketPurchaser) BalanceToMaintain() pfcutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetBalanceToMaintain(balanceToMaintain int64) {
	t.purchaserMtx.Lock()
	t.balanceToMaintain = pfcutil.Amount(balanceToMaintain)
	t.purchaserMtx.Unlock()
}

// MaxPriceAbsolute returns the max absolute price to purchase a ticket.
func (t *TicketPurchaser) MaxPriceAbsolute() pfcutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetMaxPriceAbsolute(maxPriceAbsolute int64) {
	t.purchaserMtx.Lock()
	t.maxPriceAbsolute = pfcutil.Amount(maxPriceAbsolute)
	t.purchaserMtx.Unlock()
}
...
func (t *TicketPurchaser) MaxFee() pfcutil.Amount {
	t.purchaserMtx.Lock()
	maxFee := t.maxFee
	t.purchaserMtx.Unlock()
	return maxFee
...
func (t *TicketPurchaser) SetMaxFee(maxFee int64) {
	t.purchaserMtx.Lock()
	t.maxFee = pfcutil.Amount(maxFee)
	t.purchaserMtx.Unlock()
}

// MinFee returns the min ticket fee per KB to use when purchasing tickets.
func (t *TicketPurchaser) MinFee() pfcutil.Amount {
	t.purchaserMtx.Lock()
	minFee := t.minFee
	t.purchaserMtx.Unlock()
	return minFee
...
func (t *TicketPurchaser) SetMinFee(minFee int64) {
	t.purchaserMtx.Lock()
	t.minFee = pfcutil.Amount(minFee)
	t.purchaserMtx.Unlock()
}

// VotingAddress returns the address to send ticket outputs to.
func (t *TicketPurchaser) VotingAddress() pfcutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetVotingAddress(votingAddress pfcutil.Address) {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) PoolAddress() pfcutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetPoolAddress(poolAddress pfcutil.Address) {
	t.purchaserMtx.Lock()
...
func NewTicketPurchaser(cfg *Config,
	...
	return &TicketPurchaser{
		...
		account:           account,
		balanceToMaintain: pfcutil.Amount(cfg.BalanceToMaintainAbsolute),
		maxFee:            pfcutil.Amount(cfg.MaxFee),
		minFee:            pfcutil.Amount(cfg.MinFee),
		maxPerBlock:       cfg.MaxPerBlock,
		maxPriceAbsolute:  pfcutil.Amount(cfg.MaxPriceAbsolute),
...
type PurchaseStats struct {
	...
	PriceAverage  pfcutil.Amount
	PriceNext     pfcutil.Amount
	PriceCurrent  pfcutil.Amount
	Purchased     int
	LeftWindow    int
...
func (t *TicketPurchaser) Purchase(height int64) (*PurchaseStats, error) {
	...
	if t.firstStart {
		...
		log.Debugf("Transaction relay fee: %v", pfcutil.Amount(t.cfg.TxFee))
	...
	if refreshStakeInfo {
		...
		for i := 1; i <= stakeInfoReqTries; i++ {
			curStakeInfo, err = t.wallet.StakeInfo(t.pfcdChainSvr)
			if err != nil {
		...
		t.stakeVoteSubsidy = pfcutil.Amount(subsidy)
	...
	if avgPriceAmt < pfcutil.Amount(t.activeNet.MinimumStakeDiff) {
		avgPriceAmt = pfcutil.Amount(t.activeNet.MinimumStakeDiff)
	}
	...
	sDiffEsts, err := t.pfcdChainSvr.EstimateStakeDiff(nil)
	if err == nil {
		ps.PriceNext, err = pfcutil.NewAmount(sDiffEsts.Expected)
		if err != nil {
			return ps, err
		}
	...
	var maxPriceAmt pfcutil.Amount
	...
	oneBlock := uint32(1)
	info, err := t.pfcdChainSvr.TicketFeeInfo(&oneBlock, &zeroUint32)
	if err != nil {
		return ps, err
	}
	...
	if !t.cfg.NoSpreadTicketPurchases && toBuyForBlock > 0 {
		...
		log.Debugf("Your average purchase price for tickets in the pool is %.2f PFC", yourAvgTixPrice)
		log.Debugf("Available funds of %.2f PFC can buy %.2f tickets, %.2f tickets per block",
		...
		log.Debugf("Redeemed ticket value expected is %.2f PFC, buys %.2f tickets, %.2f%% more",
			redeemedFunds, tixToBuyWithRedeemedFunds, tixToBuyWithRedeemedFunds/tixCanBuy*100)
		log.Debugf("Stake reward expected is %.2f PFC, buys %.2f tickets, %.2f%% more",
	...
	// have enough moneys.
	notEnough := func(bal pfcutil.Amount, toBuy int, sd pfcutil.Amount) bool {
	...
	// Purchase tickets.
	poolFeesAmt, err := pfcutil.NewAmount(t.cfg.PoolFees)
	if err != nil {
		return ps, err
	}
