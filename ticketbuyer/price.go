...
func (t *TicketPurchaser) calcAverageTicketPrice(height int64) (pfcutil.Amount, error) {
	...
	// these two prices.
	var avgPricePoolAmt pfcutil.Amount
	if t.priceMode == AvgPricePoolMode || t.priceMode == AvgPriceDualMode {
		...
		avgPricePoolAmt = poolValue / pfcutil.Amount(poolSize)
