...
type diffPeriodFee struct {
	difficulty pfcutil.Amount
	difference pfcutil.Amount // Difference from current difficulty
	fee        pfcutil.Amount
}
...
func (t *TicketPurchaser) findClosestFeeWindows(difficulty pfcutil.Amount,
	useMedian bool) (pfcutil.Amount, error) {
	...
	for i := range info.FeeInfoWindows {
		...
		windowDiffAmt := pfcutil.Amount(blkHeader.SBits)

		var fee pfcutil.Amount
		if !useMedian {
			fee, err = pfcutil.NewAmount(info.FeeInfoWindows[i].Mean)
		} else {
			fee, err = pfcutil.NewAmount(info.FeeInfoWindows[i].Median)
		}
		if err != nil {
			return 0, err
		}
...
func (t *TicketPurchaser) findTicketFeeBlocks(useMedian bool) (pfcutil.Amount, error) {
	...
	var sum, tmp pfcutil.Amount
	for i := range info.FeeInfoBlocks {
		if !useMedian {
			tmp, err = pfcutil.NewAmount(info.FeeInfoBlocks[i].Mean)
		} else {
			tmp, err = pfcutil.NewAmount(info.FeeInfoBlocks[i].Median)
		}
		if err != nil {
			return 0, err
		}
	...
	return sum / pfcutil.Amount(t.cfg.BlocksToAvg), nil
}
