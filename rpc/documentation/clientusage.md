...
var certificateFile = filepath.Join(dcrutil.AppDataDir("pfcwallet", false), "rpc.cert")
...
```bash
$ protoc -I/path/to/pfcwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/pfcwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.pfcwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\pfcwallet\rpc --csharp_out=Example --grpc_out=Example `
...
```bash
$ protoc -I /path/to/picfight/pfcwallet/rpc --python_out=. --grpc_out=. \
