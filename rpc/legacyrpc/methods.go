...
import (
	...
	"github.com/picfight/pfcwallet/internal/helpers"
	"github.com/picfight/pfcwallet/p2p"
	ver "github.com/picfight/pfcwallet/version"
...
// context.
type lazyHandler func() (interface{}, *pfcjson.RPCError)
...
func lazyApplyHandler(s *Server, request *pfcjson.Request) lazyHandler {
	handlerData, ok := handlers[request.Method]
	if !ok {
		return func() (interface{}, *pfcjson.RPCError) {
			...
			if err != nil {
				return nil, rpcErrorf(pfcjson.ErrRPCClientNotConnected, "RPC passthrough requires pfcd RPC synchronization")
			}
	...
	return func() (interface{}, *pfcjson.RPCError) {
		...
		if err != nil {
			return nil, pfcjson.ErrRPCInvalidRequest
		}
...
func makeResponse(id, result interface{}, err error) pfcjson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return pfcjson.Response{
			ID:    idPtr,
...
func accountAddressIndex(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.AccountAddressIndexCmd)
	...
	case udb.InternalBranch:
		return intChild, nil
	default:
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "invalid branch %v", cmd.Branch)
	}
}
...
func accountSyncAddressIndex(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.AccountSyncAddressIndexCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if index >= hdkeychain.HardenedKeyStart {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter,
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*pfcutil.AddressSecpPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			if pfcec.SignatureType(pubKey.GetType()) != pfcec.STEcdsaSecp256k1 {
				return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter,
					"only secp256k1 pubkeys are currently supported")
			}
			pubKeyAddr, err := pfcutil.NewAddressSecpPubKey(
				pubKey.Serialize(), w.ChainParams())
			if err != nil {
				return nil, rpcError(pfcjson.ErrRPCInvalidAddressOrKey, err)
			}
...
func addMultiSigAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.AddMultisigAddressCmd)
	...
	secp256k1Addrs := make([]pfcutil.Address, len(cmd.Keys))
	...
	err = n.LoadTxFilter(context.TODO(), false, []pfcutil.Address{p2shAddr}, nil)
	if err != nil {
		return nil, err
	}
...
func consolidate(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ConsolidateCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	var changeAddr pfcutil.Address
	if cmd.Address != nil {
		if *cmd.Address != "" {
...
func createMultiSig(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.CreateMultisigCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	address, err := pfcutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
		return nil, err
	}

	return pfcjson.CreateMultiSigResult{
...
func dumpPrivKey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.DumpPrivKeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func generateVote(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GenerateVoteCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getAddressesByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetAddressesByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getBalance(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetBalanceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if minConf < 0 {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "minconf must be non-negative")
	}
	...
	result := pfcjson.GetBalanceResult{
	...
	if accountName == "*" {
		...
		var (
			totImmatureCoinbase pfcutil.Amount
			totImmatureStakegen pfcutil.Amount
			totLocked           pfcutil.Amount
			totSpendable        pfcutil.Amount
			totUnconfirmed      pfcutil.Amount
			totVotingAuthority  pfcutil.Amount
			cumTot              pfcutil.Amount
		)

		balancesLen := uint32(len(balances))
		result.Balances = make([]pfcjson.GetAccountBalanceResult, balancesLen)
		...
		for _, bal := range balances {
			...
			if err != nil {
				...
				if errors.Is(errors.NotExist, err) {
					return nil, rpcError(pfcjson.ErrRPCInternal.Code, err)
				}
				return nil, err
			}
			...
			json := pfcjson.GetAccountBalanceResult{
...
func getInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	var spendableBalance pfcutil.Amount
	for _, balance := range balances {
		spendableBalance += balance.Spendable
	}

	info := &pfcjson.InfoWalletResult{
		Version:         ver.Integer,
...
func decodeAddress(s string, params *chaincfg.Params) (pfcutil.Address, error) {
	...
	if len(s) == 66 || len(s) == 130 {
		...
		pubKeyAddr, err := pfcutil.NewAddressSecpPubKey(pubKeyBytes,
			params)
		if err != nil {
			return nil, err
		}
	...
	addr, err := pfcutil.DecodeAddress(s)
	if err != nil {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidAddressOrKey,
...
func getAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
// runs out (and will return pfcjson.ErrRPCWalletKeypoolRanOut if that happens).
func getAccountAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetAccountAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getUnconfirmedBalance(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetUnconfirmedBalanceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func importPrivKey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ImportPrivKeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	wif, err := pfcutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidAddressOrKey, "WIF decode failed: %v", err)
	}
	if !wif.IsForNet(w.ChainParams()) {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidAddressOrKey, "key is not intended for %s", w.ChainParams().Name)
	}
...
func importScript(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ImportScriptCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if len(rs) == 0 {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "empty script")
	}
...
func createNewAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.CreateNewAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if err != nil {
		if errors.Is(errors.Locked, err) {
			return nil, rpcErrorf(pfcjson.ErrRPCWalletUnlockNeeded, "creating new accounts requires an unlocked wallet")
		}
		return nil, err
	}
	return nil, nil
...
func renameAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.RenameAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getMultisigOutInfo(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetMultisigOutInfoCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	result := &pfcjson.GetMultisigOutInfoResult{
...
func getNewAddress(s *Server, icmd interface{}) (interface{}, error) {
	...
	if cmd.GapPolicy != nil {
		...
		default:
			return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "unknown gap policy %q", *cmd.GapPolicy)
		}
	}
...
func getRawChangeAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetRawChangeAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getReceivedByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetReceivedByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getReceivedByAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetReceivedByAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getMasterPubkey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetMasterPubkeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getStakeInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	resp := &pfcjson.GetStakeInfoResult{
...
func getTickets(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetTicketsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	return &pfcjson.GetTicketsResult{Hashes: ticketHashStrs}, nil
}

// getTransaction handles a gettransaction request by returning details about
// a single transaction saved by wallet.
func getTransaction(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.GetTransactionCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	var (
		debitTotal  pfcutil.Amount
		creditTotal pfcutil.Amount
		fee         pfcutil.Amount
		negFeeF64   float64
	)
	...
	if len(txd.Debits) == len(txd.MsgTx.TxIn) {
		var outputTotal pfcutil.Amount
		for _, output := range txd.MsgTx.TxOut {
			outputTotal += pfcutil.Amount(output.Value)
		}
	...
	ret.Details = make([]pfcjson.GetTransactionDetailsResult, len(details))
	for i, d := range details {
		ret.Details[i] = pfcjson.GetTransactionDetailsResult{
			Account:           d.Account,
			Address:           d.Address,
			Amount:            d.Amount,
			Category:          d.Category,
...
func getVoteChoices(s *Server, icmd interface{}) (interface{}, error) {
	...
	resp := &pfcjson.GetVoteChoicesResult{
		Version: version,
		Choices: make([]pfcjson.VoteChoice, len(agendas)),
	}
	...
	for i := range choices {
		resp.Choices[i] = pfcjson.VoteChoice{
...
func help(s *Server, icmd interface{}) (interface{}, error) {
	...
	return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "no help for method %q", *cmd.Command)
}

// listAccounts handles a listaccounts request by returning a map of account
// names to their balances.
func listAccounts(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ListAccountsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listReceivedByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ListReceivedByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	jsonResults := make([]pfcjson.ListReceivedByAccountResult, 0, len(results))
	for _, result := range results {
		jsonResults = append(jsonResults, pfcjson.ListReceivedByAccountResult{
...
func listReceivedByAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ListReceivedByAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	ret := make([]pfcjson.ListReceivedByAddressResult, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = pfcjson.ListReceivedByAddressResult{
			Address:       address,
...
func listSinceBlock(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ListSinceBlockCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if targetConf < 1 {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "target_confirmations must be positive")
	}
	...
	res := &pfcjson.ListSinceBlockResult{
...
func listScripts(s *Server, icmd interface{}) (interface{}, error) {
	...
	listScriptsResultSIs := make([]pfcjson.ScriptInfo, len(redeemScripts))
	for i, redeemScript := range redeemScripts {
		p2shAddr, err := pfcutil.NewAddressScriptHash(redeemScript,
			w.ChainParams())
		if err != nil {
			return nil, err
		}
		listScriptsResultSIs[i] = pfcjson.ScriptInfo{
			Hash160:      hex.EncodeToString(p2shAddr.Hash160()[:]),
			Address:      p2shAddr.EncodeAddress(),
			RedeemScript: hex.EncodeToString(redeemScript),
		}
	}
	return &pfcjson.ListScriptsResult{Scripts: listScriptsResultSIs}, nil
}

// listTransactions handles a listtransactions request by returning an
// array of maps with details of sent and recevied wallet transactions.
func listTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ListTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listAddressTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ListAddressTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listAllTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ListAllTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listUnspent(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ListUnspentCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func lockUnspent(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.LockUnspentCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func purchaseTicket(s *Server, icmd interface{}) (interface{}, error) {
	...
	cmd := icmd.(*pfcjson.PurchaseTicketCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	spendLimit, err := pfcutil.NewAmount(cmd.SpendLimit)
	if err != nil {
		return nil, rpcError(pfcjson.ErrRPCInvalidParameter, err)
	}
	if spendLimit < 0 {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "negative spend limit")
	}
	...
	if cmd.MinConf != nil {
		minConf = int32(*cmd.MinConf)
		if minConf < 0 {
			return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "negative minconf")
		}
	}

	// Set ticket address if specified.
	var ticketAddr pfcutil.Address
	...
	if cmd.PoolAddress != nil {
		if *cmd.PoolAddress != "" {
			...
			// the pool after.
			if cmd.PoolFees == nil {
				return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "pool address set without pool fee")
			}
			poolFee = *cmd.PoolFees
			if !txrules.ValidPoolFeeRate(poolFee) {
				return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "pool fee percentage %v", poolFee)
			}
		}
	}
	...
	if cmd.TicketFee != nil {
		ticketFee, err = pfcutil.NewAmount(*cmd.TicketFee)
		if err != nil {
			return nil, rpcError(pfcjson.ErrRPCInvalidParameter, err)
		}
	}
...
// and amounts.
func makeOutputs(pairs map[string]pfcutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
// All errors are returned in pfcjson.RPCError format
func sendPairs(w *wallet.Wallet, amounts map[string]pfcutil.Amount, account uint32, minconf int32) (string, error) {
	...
	if err != nil {
		...
		if errors.Is(errors.InsufficientBalance, err) {
			return "", rpcError(pfcjson.ErrRPCWalletInsufficientFunds, err)
		}
		return "", err
	}
...
func redeemMultiSigOut(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.RedeemMultiSigOutCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	srtTyped := signedTxResult.(pfcjson.SignRawTransactionResult)
	return pfcjson.RedeemMultiSigOutResult(srtTyped), nil
}
...
func redeemMultiSigOuts(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.RedeemMultiSigOutsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	p2shAddr, ok := addr.(*pfcutil.AddressScriptHash)
	if !ok {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "address is not P2SH")
	}
	...
	itr := uint32(0)
	rmsoResults := make([]pfcjson.RedeemMultiSigOutResult, len(msos))
	for i, mso := range msos {
		if itr > max {
			break
		}

		rmsoRequest := &pfcjson.RedeemMultiSigOutCmd{
		...
		redeemResultTyped := redeemResult.(pfcjson.RedeemMultiSigOutResult)
		rmsoResults[i] = redeemResultTyped

		itr++
	}

	return pfcjson.RedeemMultiSigOutsResult{Results: rmsoResults}, nil
}
...
func rescanWallet(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.RescanWalletCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func stakePoolUserInfo(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.StakePoolUserInfoCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	userAddr, err := pfcutil.DecodeAddress(cmd.User)
	if err != nil {
		return nil, err
	}
...
func ticketsForAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.TicketsForAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	addr, err := pfcutil.DecodeAddress(cmd.Address)
	if err != nil {
		return nil, err
	}
	...
	return pfcjson.TicketsForAddressResult{Tickets: ticketHashStrs}, nil
}
...
func sendFrom(s *Server, icmd interface{}) (interface{}, error) {
	...
	if !isNilOrEmpty(cmd.Comment) || !isNilOrEmpty(cmd.CommentTo) {
		return nil, rpcErrorf(pfcjson.ErrRPCUnimplemented, "transaction comments are unsupported")
	}
	...
	if cmd.Amount < 0 {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "negative amount")
	}
	minConf := int32(*cmd.MinConf)
	if minConf < 0 {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "negative minconf")
	...
	amt, err := pfcutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(pfcjson.ErrRPCInvalidParameter, err)
	}
	pairs := map[string]pfcutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(s *Server, icmd interface{}) (interface{}, error) {
	...
	// Recreate address/amount pairs, using pfcutil.Amount.
	pairs := make(map[string]pfcutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := pfcutil.NewAmount(v)
		if err != nil {
			return nil, rpcError(pfcjson.ErrRPCInvalidParameter, err)
		}
		pairs[k] = amt
...
func sendToAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.SendToAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func sendToMultiSig(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.SendToMultiSigCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	account := uint32(udb.DefaultAccountNum)
	amount, err := pfcutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(pfcjson.ErrRPCInvalidParameter, err)
	}
	nrequired := int8(*cmd.NRequired)
	minconf := int32(*cmd.MinConf)
	pubkeys := make([]*pfcutil.AddressSecpPubKey, len(cmd.Pubkeys))
	...
	result := &pfcjson.SendToMultiSigResult{
...
func setTicketFee(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.SetTicketFeeCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if cmd.Fee < 0 {
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "negative fee")
	}

	incr, err := pfcutil.NewAmount(cmd.Fee)
	if err != nil {
		return nil, rpcError(pfcjson.ErrRPCInvalidParameter, err)
	}
...
func setTxFee(s *Server, icmd interface{}) (interface{}, error) {
	...
	relayFee, err := pfcutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(pfcjson.ErrRPCInvalidParameter, err)
	}
...
func setVoteChoice(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.SetVoteChoiceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func signMessage(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.SignMessageCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func signRawTransaction(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.SignRawTransactionCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	case "ssrtx": // Special case of SigHashAll
		hashType = txscript.SigHashAll
	default:
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "invalid sighash flag")
	}
	...
	var cmdInputs []pfcjson.RawTxInput
	if cmd.Inputs != nil {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := pfcutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, err
			}
	...
	var keys map[string]*pfcutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*pfcutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := pfcutil.DecodeWIF(key)
			if err != nil {
				return nil, rpcError(pfcjson.ErrRPCDeserialization, err)
			}

			if !wif.IsForNet(w.ChainParams()) {
				return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "key intended for different network")
			}

			var addr pfcutil.Address
			switch wif.DSA() {
			case pfcec.STEcdsaSecp256k1:
				addr, err = pfcutil.NewAddressSecpPubKey(wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
				}
			case pfcec.STEd25519:
				addr, err = pfcutil.NewAddressEdwardsPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
			...
			case pfcec.STSchnorrSecp256k1:
				addr, err = pfcutil.NewAddressSecSchnorrPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
	...
	signErrors := make([]pfcjson.SignRawTransactionError, 0, len(signErrs))
	for _, e := range signErrs {
		input := tx.TxIn[e.InputIndex]
		signErrors = append(signErrors, pfcjson.SignRawTransactionError{
...
func startAutoBuyer(s *Server, icmd interface{}) (interface{}, error) {
	...
	case poolFees == 0 && poolAddress != nil:
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "pooladdress set without poolfees")
	case poolFees != 0 && poolAddress == nil:
		return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "poolfees set without pooladdress")
	case poolFees != 0 && poolAddress != nil:
		if !txrules.ValidPoolFeeRate(poolFees) {
			return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "invalid poolfees %v", poolFees)
		}
	}
...
func sweepAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.SweepAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if cmd.FeePerKb != nil {
		var err error
		feePerKb, err = pfcutil.NewAmount(*cmd.FeePerKb)
		if err != nil {
			return nil, rpcError(pfcjson.ErrRPCInvalidParameter, err)
		}
	}
	...
	if err != nil {
		if errors.Is(errors.InsufficientBalance, err) {
			return nil, rpcError(pfcjson.ErrRPCWalletInsufficientFunds, err)
		}
		return nil, err
	}
...
func validateAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.ValidateAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	result := pfcjson.ValidateAddressWalletResult{}
	addr, err := decodeAddress(cmd.Address, w.ChainParams())
	if err != nil {
...
func verifyMessage(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.VerifyMessageCmd)
	...
	case *pfcutil.AddressPubKeyHash:
		if a.DSA(a.Net()) != pfcec.STEcdsaSecp256k1 {
			goto WrongAddrKind
		}
	default:
		goto WrongAddrKind
...
WrongAddrKind:
	return nil, rpcErrorf(pfcjson.ErrRPCInvalidParameter, "address must be secp256k1 P2PK or P2PKH")
}
...
func version(s *Server, icmd interface{}) (interface{}, error) {
	var resp map[string]pfcjson.VersionResult
	n, _ := s.walletLoader.NetworkBackend()
	chainClient, err := chain.RPCClientFromBackend(n)
	if err == nil {
		var err error
		...
		resp = make(map[string]pfcjson.VersionResult)
	}

	resp["pfcwalletjsonrpcapi"] = pfcjson.VersionResult{
...
func walletInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	return &pfcjson.WalletInfoResult{
...
func walletPassphrase(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.WalletPassphraseCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func walletPassphraseChange(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*pfcjson.WalletPassphraseChangeCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if err != nil {
		if errors.Is(errors.Passphrase, err) {
			return nil, rpcErrorf(pfcjson.ErrRPCWalletPassphraseIncorrect, "incorrect passphrase")
		}
		return nil, err
	}
	return nil, nil
...
func decodeHexStr(hexStr string) ([]byte, error) {
	...
	if err != nil {
		return nil, rpcErrorf(pfcjson.ErrRPCDecodeHexString, "hex string decode failed: %v", err)
	}
	return decoded, nil
}
