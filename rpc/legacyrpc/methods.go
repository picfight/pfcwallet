...
// request into a marshalable response.  If the error is a *pfcjson.RPCError
...
// catch-all error code, pfcjson.ErrRPCWallet.
...
// unsupported handles a standard picfightcoind RPC request which is
...
func lazyApplyHandler(request *pfcjson.Request, w *wallet.Wallet, chainClient chain.Interface) lazyHandler {
...
func jsonError(err error) *pfcjson.RPCError {
	if err == nil {
		return nil
	}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*pfcutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := pfcutil.NewAddressPubKey(
...
// TODO: finish this to match picfightcoind by writing the dump to a file.
...
func decodeAddress(s string, params *chaincfg.Params) (pfcutil.Address, error) {
	addr, err := pfcutil.DecodeAddress(s, params)
	if err != nil {
...
func getUnconfirmedBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	return (bals.Total - bals.Spendable).ToPFC(), nil
}
...
//
// Note: picfightcoind allows specifying the account as an optional parameter,
...
func getReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	return results[acctIndex].TotalReceived.ToPFC(), nil
}
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	var (
		...
		creditTotal pfcutil.Amount // Excludes change
		fee         pfcutil.Amount
		feeF64      float64
	)
	...
	if len(details.Debits) == 0 {
		...
		ret.Details = make([]pfcjson.GetTransactionDetailsResult, 0, len(details.Credits))
	} else {
		ret.Details = make([]pfcjson.GetTransactionDetailsResult, 1, len(details.Credits)+1)

		ret.Details[0] = pfcjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Account
			//   Address
			...
			Category: "send",
			Amount:   (-debitTotal).ToPFC(), // negative since it is a send
			Fee:      &feeF64,
		}
		ret.Fee = feeF64
	}
	...
	for _, cred := range details.Credits {
		...
		ret.Details = append(ret.Details, pfcjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Fee
			Account:  accountName,
			Address:  address,
			Category: credCat,
			Amount:   cred.Amount.ToPFC(),
			Vout:     cred.Index,
		})
	}

	ret.Amount = creditTotal.ToPFC()
	return ret, nil
}
...
func help(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// pfcd returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to pfcd.  Therefore, create a
	...
	// texts in a non-internal pfcd package.
...
func listAccounts(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, result := range results {
		accountBalances[result.AccountName] = result.AccountBalance.ToPFC()
	}
...
func listReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, result := range results {
		jsonResults = append(jsonResults, pfcjson.ListReceivedByAccountResult{
			...
			Amount:        result.TotalReceived.ToPFC(),
...
func listReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	ret := make([]pfcjson.ListReceivedByAddressResult, numAddresses, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = pfcjson.ListReceivedByAddressResult{
			Address:       address,
			Amount:        addrData.amount.ToPFC(),
...
func makeOutputs(pairs map[string]pfcutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	...
	for addrStr, amt := range pairs {
		addr, err := pfcutil.DecodeAddress(addrStr, chainParams)
		if err != nil {
...
func sendPairs(w *wallet.Wallet, amounts map[string]pfcutil.Amount,
	account uint32, minconf int32, feeSatPerKb pfcutil.Amount) (string, error) {
...
func sendToAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	// sendtoaddress always spends from the default account, this matches picfightcoind
...
func signMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	sigbytes, err := pfcec.SignCompact(pfcec.S256(), privKey,
		messageHash, true)
	if err != nil {
		return nil, err
	}
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	if cmd.PrivKeys != nil {
		...
		for _, key := range *cmd.PrivKeys {
			...
			addr, err := pfcutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	case *pfcutil.AddressPubKeyHash: // ok
		return bytes.Equal(pfcutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *pfcutil.AddressPubKey: // ok
