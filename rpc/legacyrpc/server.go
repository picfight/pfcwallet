...
// functional picfight wallet RPC server.  This can be called to enable RPC
...
// fully functional picfight wallet RPC server.
...
// method.  This may be a request that is handled directly by pfcwallet, or
// a chain server request that is handled by passing the request down to pfcd.
//
...
// but this is only implemented here to work around an oddity with pfcjson,
...
func (s *Server) invalidAuth(req *pfcjson.Request) bool {
	cmd, err := pfcjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*pfcjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
