...
func TestMain(m *testing.M) {
	...
	ntfnHandlersNode := pfcrpcclient.NotificationHandlers{
...
func testValidateAddress(r *Harness, t *testing.T) {
	...
	for _, acct := range accounts {
		...
		otherAddress, err := pfcutil.DecodeAddress("SsqvxBX8MZC5iiKCgBscwt69jg4u4hHhDKU")
		if err != nil {
...
func testSendFrom(r *Harness, t *testing.T) {
	...
	fee := pfcutil.Amount(totalSpent - totalSent)
...
func testListTransactions(r *Harness, t *testing.T) {
	...
	sendAmount := pfcutil.Amount(240000000)
...
func testGetSetRelayFee(r *Harness, t *testing.T) {
	// pfcrpcclient does not have a getwalletfee or any direct method, so we
...
func testGetSetTicketFee(r *Harness, t *testing.T) {
	// pfcrpcclient does not have a getticketee or any direct method, so we
...
func mustGetStakeInfo(wcl *pfcrpcclient.Client, t *testing.T) *pfcjson.GetStakeInfoResult {
	stakeinfo, err := wcl.GetStakeInfo()
	if err != nil {
		t.Fatal("GetStakeInfo failed: ", err)
	}
