...
type configJSON struct {
	...
	PfcctlArgs    string `json:"pfcctlargs"`
}
...
func convertJSONUnspentToOutPoints(
	utxos []pfcjson.ListUnspentResult) []*extendedOutPoint {
	...
	for _, utxo := range utxos {
		...
		eop.op = op
		amtCast, _ := pfcutil.NewAmount(utxo.Amount)
...
func main() {
	...
	var utxos []pfcjson.ListUnspentResult
	...
	sendToAddress, err := pfcutil.DecodeAddress(cfg.SendToAddress)
	if err != nil {
	...
	buf.WriteString("pfcctl ")
	buf.WriteString(cfg.PfcctlArgs)
