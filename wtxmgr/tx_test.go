...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := pfcutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: pfcutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: pfcutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: pfcutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance pfcutil.Amount,
	...
	checkBalance(pfcutil.Amount(initialBalance), false)
	checkBalance(pfcutil.Amount(initialBalance), true)
	...
	checkBalance(pfcutil.Amount(changeAmount), true)
