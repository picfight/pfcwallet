...
func TestPfcAddressSerializer(t *testing.T) {
...
func TestChaining(t *testing.T) {
	...
	for _, test := range tests {
		...
		// Public keys (used to generate the picfightcoin address) MUST match.
		...
		// the next pubkeys.
		pubkeyUncompressed, err := pfcec.ParsePubKey(nextPubUncompressedFromPub, pfcec.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next uncompressed pubkey: %v", test.name, err)
			return
		}
		pubkeyCompressed, err := pfcec.ParsePubKey(nextPubCompressedFromPub, pfcec.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next compressed pubkey: %v", test.name, err)
			return
		}
		privkeyUncompressed := &pfcec.PrivateKey{
			PublicKey: *pubkeyUncompressed.ToECDSA(),
			D:         new(big.Int).SetBytes(nextPrivUncompressed),
		}
		privkeyCompressed := &pfcec.PrivateKey{
		...
		if !ok {
			t.Errorf("%s: pfcec signature verification failed for next keypair (chained from uncompressed pubkey).",
				test.name)
			return
		}
		...
		if !ok {
			t.Errorf("%s: pfcec signature verification failed for next keypair (chained from compressed pubkey).",
				test.name)
			return
		}
	}
...
func TestWalletPubkeyChaining(t *testing.T) {
	...
	if !ok {
		t.Errorf("pfcec signature verification failed; address's pubkey mismatches the privkey.")
		return
	}
	...
	ok = sig.Verify(hash, pubKey)
	if !ok {
		t.Errorf("pfcec signature verification failed; next address's keypair does not match.")
		return
	}
...
func TestWatchingWalletExport(t *testing.T) {
	...
	pk, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), make([]byte, 32))
	wif, err := pfcutil.NewWIF(pk, tstNetParams, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestImportPrivateKey(t *testing.T) {
	...
	// import priv key
	wif, err := pfcutil.NewWIF((*pfcec.PrivateKey)(pk), tstNetParams, false)
	if err != nil {
		t.Fatal(err)
	}
