...
import (
	...
	"github.com/picfight/pfcwallet/internal/legacy/rename"
)
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := pfcec.PrivKeyFromBytes(pfcec.S256(), privkey)
...
func chainedPrivKey(privkey, pubkey, chaincode []byte) ([]byte, error) {
	...
	case pfcec.PubKeyBytesLenUncompressed, pfcec.PubKeyBytesLenCompressed:
		// Correct length
	default:
...
func chainedPubKey(pubkey, chaincode []byte) ([]byte, error) {
	var compressed bool
	switch n := len(pubkey); n {
	case pfcec.PubKeyBytesLenUncompressed:
		compressed = false
	...
	oldPk, err := pfcec.ParsePubKey(pubkey, pfcec.S256())
	if err != nil {
		return nil, err
	}
	newX, newY := pfcec.S256().ScalarMult(oldPk.X, oldPk.Y, xorbytes)
	if err != nil {
		return nil, err
	}
	newPk := &pfcec.PublicKey{
		Curve: pfcec.S256(),
		X:     newX,
		Y:     newY,
	}
...
func (net *netParams) ReadFrom(r io.Reader) (int64, error) {
	...
	switch wire.PicfightcoinNet(binary.LittleEndian.Uint32(uint32Bytes)) {
	case wire.MainNet:
...
func getAddressKey(addr pfcutil.Address) addressKey {
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]pfcutil.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]pfcutil.Address),
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]pfcutil.Address)
...
func (s *Store) ChangeAddress(bs *BlockStamp) (pfcutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) LastChainedAddress() pfcutil.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) Address(a pfcutil.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) SetSyncStatus(a pfcutil.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *pfcutil.WIF, bs *BlockStamp) (pfcutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := pfcutil.Hash160(wif.SerializePubKey())
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (pfcutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(pfcutil.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
type WalletAddress interface {
	// Address returns a pfcutil.Address for the backing address.
...
func (s *Store) ActiveAddresses() map[pfcutil.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[pfcutil.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the pfcutil.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]pfcutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (af *addrFlags) ReadFrom(r io.Reader) (int64, error) {
	...
	// pfcwallet shall refuse to open any unencrypted addresses.  This
...
// that pfcwallet fits into an unused space left by Armory's key store file
// format.
...
type PubKeyAddress interface {
	...
	PrivKey() (*pfcec.PrivateKey, error)
...
func newBtcAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *btcAddress, err error) {
	...
	pk, err := pfcec.ParsePubKey(pubkey, pfcec.S256())
	if err != nil {
		return nil, err
	}

	address, err := pfcutil.NewAddressPubKeyHash(pfcutil.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
func (a *btcAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	addr, err := pfcutil.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *btcAddress) unlock(key []byte) (privKeyCT []byte, err error) {
	...
	x, y := pfcec.S256().ScalarBaseMult(privkey)
...
func (a *btcAddress) ExportPrivKey() (*pfcutil.WIF, error) {
	...
	// elsewhere.
	wif, err := pfcutil.NewWIF((*pfcec.PrivateKey)(pk), a.store.netParams(),
		a.Compressed())
	if err != nil {
		panic(err)
	}
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	scriptHash := pfcutil.Hash160(script)

	address, err := pfcutil.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
func (sa *scriptAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	address, err := pfcutil.NewAddressScriptHashFromHash(scriptHash[:],
		sa.store.netParams())
	if err != nil {
		return n, err
	}
...
func (sa *scriptAddress) Addresses() []pfcutil.Address {
	return sa.addresses
}
