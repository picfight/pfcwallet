...
// style of picfightcoin addresses.  It additionally provides information about the
// script.
...
// Address returns the pfcutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() pfcutil.Address {
	return a.address
}
...
func (a *managedAddress) PubKey() *pfcec.PublicKey {
	return a.pubKey
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) PrivKey() (*pfcec.PrivateKey, error) {
	...
	privKey, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), privKeyCopy)
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*pfcutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return pfcutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	derivationPath DerivationPath, pubKey *pfcec.PublicKey, compressed bool,
	...
	case NestedWitnessPubKey:
		...
		// backwards compatible to Picfightcoin nodes running 0.6.0 onwards, but
		...
		witAddr, err := pfcutil.NewAddressWitnessPubKeyHash(
		...
		address, err = pfcutil.NewAddressScriptHash(
	...
	case WitnessPubKey:
		address, err = pfcutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
func newManagedAddress(s *ScopedKeyManager, derivationPath DerivationPath,
	privKey *pfcec.PrivateKey, compressed bool,
	addrType AddressType) (*managedAddress, error) {
	...
	ecPubKey := (*pfcec.PublicKey)(&privKey.PublicKey)
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *pfcutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() pfcutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := pfcutil.NewAddressScriptHashFromHash(
