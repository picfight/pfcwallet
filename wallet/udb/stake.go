...
func (s *StakeStore) insertSStx(ns walletdb.ReadWriteBucket, sstx *pfcutil.Tx) error {
...
func (s *StakeStore) dumpSStxHashesForAddress(ns walletdb.ReadBucket, addr pfcutil.Address) ([]chainhash.Hash, error) {
	// Extract the HASH160 script hash; if it's not 20 bytes
	// long, return an error.
	hash160 := addr.ScriptAddress()
	_, addrIsP2SH := addr.(*pfcutil.AddressScriptHash)
...
func (s *StakeStore) sstxAddress(ns walletdb.ReadBucket, hash *chainhash.Hash) (pfcutil.Address, error) {
	// Access the database and store the result locally.
...
func (s *StakeStore) updateStakePoolUserTickets(ns walletdb.ReadWriteBucket, user pfcutil.Address, ticket *PoolTicket) error {
	_, isScriptHash := user.(*pfcutil.AddressScriptHash)
	_, isP2PKH := user.(*pfcutil.AddressPubKeyHash)
...
func (s *StakeStore) removeStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user pfcutil.Address, ticket *chainhash.Hash) error {
	_, isScriptHash := user.(*pfcutil.AddressScriptHash)
	_, isP2PKH := user.(*pfcutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		return errors.E(errors.Invalid, errors.Errorf("voting address type %T", user))
...
// database, it adds it.
func (s *StakeStore) updateStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user pfcutil.Address, ticket *chainhash.Hash) error {
	_, isScriptHash := user.(*pfcutil.AddressScriptHash)
	_, isP2PKH := user.(*pfcutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		return errors.E(errors.Invalid, errors.Errorf("voting address type %T", user))
...
func stakePoolUserInfo(ns walletdb.ReadBucket, user pfcutil.Address) (*StakePoolUser, error) {
	_, isScriptHash := user.(*pfcutil.AddressScriptHash)
	_, isP2PKH := user.(*pfcutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
...
func (s *StakeStore) StakePoolUserInfo(ns walletdb.ReadBucket, user pfcutil.Address) (*StakePoolUser, error) {
