...
type Store struct {
	...
	acctLookupFunc func(walletdb.ReadBucket, pfcutil.Address) (uint32, error)
}
...
// BlockCFilter is a compact filter for a PicFight block.
...
func stakeValidate(ns walletdb.ReadWriteBucket, height int32) error {
	...
	for i := range blockRec.transactions {
		...
		for i, output := range txRec.MsgTx.TxOut {
			...
			minedBalance += pfcutil.Amount(output.Value)
		}
...
func (s *Store) AddCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if invalidated {
		...
		cred := credit{
			...
			block:      block.Block,
			amount:     pfcutil.Amount(rec.MsgTx.TxOut[index].Value),
			change:     change,
			spentBy:    indexedIncidence{index: ^uint32(0)},
...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if block == nil {
		...
		v := valueUnminedCredit(pfcutil.Amount(rec.MsgTx.TxOut[index].Value),
	...
	txOutAmt := pfcutil.Amount(rec.MsgTx.TxOut[index].Value)
...
func (s *Store) AddMultisigOut(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta, index uint32) error {
	...
	val = valueMultisigOut(p2shScriptHash,
		...
		block.Block.Hash,
		uint32(block.Block.Height),
		pfcutil.Amount(rec.MsgTx.TxOut[index].Value),
		*empty,     // Unspent
		0xFFFFFFFF, // Unspent
		rec.Hash)
...
func (s *Store) UnspentMultisigCreditsForAddress(ns walletdb.ReadBucket, addr pfcutil.Address) ([]*MultisigCredit, error) {
...
// without error.
func (s *Store) UnspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket, needed pfcutil.Amount, syncHeight int32, minConf int32, all bool, account uint32) ([]*Credit, error) {
	var eligible []*minimalCredit
	var toUse []*minimalCredit
	var unspent []*Credit
	found := pfcutil.Amount(0)
...
type InputSource struct {
	source func(pfcutil.Amount) (*txauthor.InputDetail, error)
}
...
func (s *InputSource) SelectInputs(target pfcutil.Amount) (*txauthor.InputDetail, error) {
...
func (s *Store) MakeInputSource(ns, addrmgrNs walletdb.ReadBucket, account uint32, minConf, syncHeight int32) InputSource {
	...
	var (
		currentTotal      pfcutil.Amount
		currentInputs     []*wire.TxIn
		currentScripts    [][]byte
		redeemScriptSizes []int
	)

	f := func(target pfcutil.Amount) (*txauthor.InputDetail, error) {
...
func (s *Store) balanceFullScan(ns, addrmgrNs walletdb.ReadBucket, minConf int32, syncHeight int32) (map[uint32]*Balances, error) {
	...
	for k, v := c.First(); k != nil; k, v = c.Next() {
		...
		case txscript.OP_SSTX:
			...
			votingAuthorityAmt := pfcutil.Amount(0)
			lockedByTicketsAmt := pfcutil.Amount(0)
			...
			totalInputAmount := pfcutil.Amount(0)
			...
			for _, txin := range rec.MsgTx.TxIn {
				...
				if rawUnmined != nil {
					...
					if int(txin.PreviousOutPoint.Index) < len(tx.TxOut) {
						totalInputAmount += pfcutil.Amount(tx.TxOut[txin.PreviousOutPoint.Index].Value)
					}
				} else {
...
type Balances struct {
	Account                 uint32
	ImmatureCoinbaseRewards pfcutil.Amount
	ImmatureStakeGeneration pfcutil.Amount
	LockedByTickets         pfcutil.Amount
	Spendable               pfcutil.Amount
	Total                   pfcutil.Amount
	VotingAuthority         pfcutil.Amount
	Unconfirmed             pfcutil.Amount
}
...
func (s *Store) TotalInput(dbtx walletdb.ReadTx, tx *wire.MsgTx) (pfcutil.Amount, error) {
	ns := dbtx.ReadBucket(wtxmgrBucketKey)
	...
	for _, in := range tx.TxIn {
		...
		total += pfcutil.Amount(tx.TxOut[in.PreviousOutPoint.Index].Value)
	}
