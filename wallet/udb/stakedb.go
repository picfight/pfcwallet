...
func deserializeSStxRecord(serializedSStxRecord []byte, dbVersion uint32) (*sstxRecord, error) {
	...
	case dbVersion < 3:
		...
		// Create and save the pfcutil.Tx of the read MsgTx and set its index.
		tx := pfcutil.NewTx(msgTx)
		tx.SetIndex(pfcutil.TxIndexUnknown)
	...
	case dbVersion >= 3:
		...
		return &sstxRecord{tx: pfcutil.NewTx(&tx), ts: time.Unix(unixTime, 0)}, nil
