...
func makeTxSummary(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails) TransactionSummary {
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		...
		for _, txOut := range details.MsgTx.TxOut {
			fee -= pfcutil.Amount(txOut.Value)
		}
	}
...
func totalBalances(dbtx walletdb.ReadTx, w *Wallet, m map[uint32]pfcutil.Amount) error {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
...
func flattenBalanceMap(m map[uint32]pfcutil.Amount) []AccountBalance {
...
func relevantAccounts(w *Wallet, m map[uint32]pfcutil.Amount, txs []TransactionSummary) {
	for _, tx := range txs {
