...
const DefaultRelayFeePerKb pfcutil.Amount = 1e5
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb pfcutil.Amount) bool {
	...
	return IsDustAmount(pfcutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
func FeeForSerializeSize(relayFeePerKb pfcutil.Amount, txSerializeSize int) pfcutil.Amount {
	fee := relayFeePerKb * pfcutil.Amount(txSerializeSize) / 1000
