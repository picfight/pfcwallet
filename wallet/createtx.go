...
// account.
func (w *Wallet) NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb pfcutil.Amount, account uint32, minConf int32,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		case OutputSelectionAlgorithmAll:
			...
			inputSource = func(pfcutil.Amount) (*txauthor.InputDetail, error) {
				inputDetail, err := sourceImpl.SelectInputs(pfcutil.MaxAmount)
...
func (s *secretSource) GetKey(addr pfcutil.Address) (chainec.PrivateKey, bool, error) {
...
func (s *secretSource) GetScript(addr pfcutil.Address) ([]byte, error) {
...
func (w *Wallet) checkHighFees(totalInput pfcutil.Amount, tx *wire.MsgTx) error {
	if w.AllowHighFees {
		return nil
	}
...
//
// PicFight: This func also sends the transaction, and if successful, inserts it
...
func (w *Wallet) txToOutputsInternal(op errors.Op, outputs []*wire.TxOut, account uint32, minconf int32,
	n NetworkBackend, randomizeChangeIdx bool, txFee pfcutil.Amount) (*txauthor.AuthoredTx, error) {
	...
	if atx.ChangeIndex >= 0 && account == udb.ImportedAddrAccount {
		changeAmount := pfcutil.Amount(atx.Tx.TxOut[atx.ChangeIndex].Value)
...
func (w *Wallet) txToMultisig(op errors.Op, account uint32, amount pfcutil.Amount, pubkeys []*pfcutil.AddressSecpPubKey,
	nRequired int8, minconf int32) (*CreatedTx, pfcutil.Address, []byte, error) {
...
func (w *Wallet) txToMultisigInternal(op errors.Op, dbtx walletdb.ReadWriteTx, account uint32, amount pfcutil.Amount,
	pubkeys []*pfcutil.AddressSecpPubKey, nRequired int8, minconf int32) (*CreatedTx, pfcutil.Address, []byte, error) {

	addrmgrNs := dbtx.ReadWriteBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadWriteBucket(wtxmgrNamespaceKey)

	txToMultisigError := func(err error) (*CreatedTx, pfcutil.Address, []byte, error) {
	...
	scAddr, err := pfcutil.NewAddressScriptHash(msScript, w.chainParams)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
	...
	// Request updates from pfcd for new transactions sent to this
	// script hash address.
	err = n.LoadTxFilter(context.TODO(), false, []pfcutil.Address{scAddr}, nil)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
...
func (w *Wallet) compressWallet(op errors.Op, maxNumIns int, account uint32, changeAddr pfcutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) compressWalletInternal(op errors.Op, dbtx walletdb.ReadWriteTx, maxNumIns int, account uint32,
	changeAddr pfcutil.Address) (*chainhash.Hash, error) {
...
// passed.
func makeTicket(params *chaincfg.Params, inputPool *extendedOutPoint, input *extendedOutPoint, addrVote pfcutil.Address,
	addrSubsidy pfcutil.Address, ticketCost int64, addrPool pfcutil.Address) (*wire.MsgTx, error) {
	...
	zeroed := [20]byte{}
	addrZeroed, err := pfcutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return nil, err
	}
	...
	if addrPool != nil {
		pkScript, err = txscript.GenerateSStxAddrPush(addrPool,
			pfcutil.Amount(amountsCommitted[0]), limits)
		if err != nil {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addrSubsidy,
		pfcutil.Amount(amountsCommitted[userSubsidyNullIdx]), limits)
	if err != nil {
		return nil, errors.E(errors.Op("txscript.GenerateSStxAddrPush"), errors.Invalid,
...
func (w *Wallet) purchaseTickets(op errors.Op, req purchaseTicketRequest) ([]*chainhash.Hash, error) {
	...
	if w.addressReuse {
		...
		addrFunc = func(errors.Op, persistReturnedChildFunc, uint32) (pfcutil.Address, error) {
			return addr, err
		}
	}
	...
	if req.minBalance > 0 {
		...
		estimatedFundsUsed := neededPerTicket * pfcutil.Amount(req.numTickets)
	...
	for i := 0; i < req.numTickets; i++ {
		...
		// an address.
		var addrVote, addrSubsidy pfcutil.Address
		err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		if eopPool != nil {
			eopPoolCredit := udb.Credit{
				...
				Amount:       pfcutil.Amount(eopPool.amt),
		...
		err = w.checkHighFees(pfcutil.Amount(eop.amt), ticket)
		if err != nil {
			return ticketHashes, errors.E(op, err)
		}
...
func (w *Wallet) findEligibleOutputsAmount(dbtx walletdb.ReadTx, account uint32, minconf int32,
	amount pfcutil.Amount, currentHeight int32) ([]udb.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)
	var outTotal pfcutil.Amount
...
func (w *Wallet) signVoteOrRevocation(addrmgrNs walletdb.ReadBucket, ticketPurchase, tx *wire.MsgTx, isVote bool) error {
	...
	// can be performed.
	var getKey txscript.KeyClosure = func(addr pfcutil.Address) (chainec.PrivateKey, bool, error) {
		...
		return key, true, nil // secp256k1 pubkeys are always compressed in PicFight
	}
	var getScript txscript.ScriptClosure = func(addr pfcutil.Address) ([]byte, error) {
	...
	signedScript, err := txscript.SignTxOutput(w.chainParams, tx, inputToSign,
		...
		tx.TxIn[inputToSign].SignatureScript, pfcec.STEcdsaSecp256k1)
	if err != nil {
...
func createUnsignedRevocation(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx, feePerKB pfcutil.Amount) (*wire.MsgTx, error) {
	// Parse the ticket purchase transaction to determine the required output
	// destinations for vote rewards or revocations.
	ticketPayKinds, ticketHash160s, ticketValues, _, _, _ :=
		stake.TxSStxStakeOutputInfo(ticketPurchase)
	...
	for _, output := range revocation.TxOut {
		if pfcutil.Amount(output.Value) > feeEstimate {
			amount := pfcutil.Amount(output.Value) - feeEstimate
