...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentInputValues := make([]pfcutil.Amount, 0, len(eligible))

	return func(target pfcutil.Amount) (pfcutil.Amount, []*wire.TxIn,
		[]pfcutil.Amount, [][]byte, error) {
...
func (s secretSource) GetKey(addr pfcutil.Address) (*pfcec.PrivateKey, bool, error) {
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb pfcutil.Amount, dryRun bool) (
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := pfcutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []pfcutil.Amount) error {
